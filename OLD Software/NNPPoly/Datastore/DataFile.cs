...
namespace NNPPoly
{
    public class DataFile
    {
        ...
        {
            eXML xml = new eXML();
            bool fl = xml.Write(Datastore.DataFile, typeof(DataFile), this, true);
            Error = xml.Error;
            return fl;
        }

        public static DataFile Read()
        {
            DataFile df = null;
            eXML xml = new eXML();
            xml.Read<DataFile>(Datastore.DataFile, typeof(DataFile), ref df, true);
            Error = xml.Error;
            return df;
        }
    }
