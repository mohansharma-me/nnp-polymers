...
namespace NNPPoly.forms
{
    public partial class frmSchemeData : Form
    {
        ...
        public void loadSchemeData()
        {
            ...
            cmbGroups.Focus();
            btnLoad.Focus();
            long groupId = cmbGroups.SelectedIndex == -1 ? 0 : (long)(cmbGroups.SelectedItem as ComboItem).Value;
            DateTime? fromDate = null;
            DateTime? toDate = null;
            if (chkDateFilter.Checked)
            {
                fromDate = new DateTime(dtFrom.Value.Year, dtFrom.Value.Month, dtFrom.Value.Day, 12, 0, 0);
                toDate = new DateTime(dtTo.Value.Year, dtTo.Value.Month, dtTo.Value.Day, 12, 0, 0);
            }
            long clientId = cmbClients.SelectedIndex == -1 ? 0 : (long)(cmbClients.SelectedItem as ComboItem).Value;
            ...
            System.Threading.Thread thread = new System.Threading.Thread(() =>
            {
                ...
                List<classes.SchemeData> datas = Job.Schemes.getAllSchemeData(groupId, clientId, fromDate, toDate, true);
        ...
        private void loadComboItems()
        {
            System.Threading.Thread thread = new System.Threading.Thread(() =>
            {
                ...
                {
                    Invoke(new Action(() => {
                        if (!groupIds.Contains(grade.group.id))
                        {
                            cmbGroups.Items.Add(new ComboItem(grade.group.name, grade.group.id));
                            groupIds.Add(grade.group.id);
                        }
        ...
        private void frmSchemeData_Load(object sender, EventArgs e)
        {
